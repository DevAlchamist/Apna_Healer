...
import { AdminClubCreationReviewModal } from "@/components/admin/admin-club-creation-review-modal";
import { AdminCreateClubModal } from "@/components/admin/admin-create-club-modal";
import { AdminEditClubModal } from "@/components/admin/admin-edit-club-modal";
...
function sphereBadgeClass(sphere: string): string {
  ...
  return "bg-[#dceee6] text-theme-status-success";
}
...
export function AdminClubsPage() {
  ...
  const [editClub, setEditClub] = useState<ApiClubDetail | null>(null);
  ...
  const [creationReviewRequest, setCreationReviewRequest] = useState<ApiClubCreationRequest | null>(
    null,
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="overflow-hidden rounded-[20px] border border-theme-muted bg-white shadow-[0_12px_40px_-28px_rgba(0,0,0,0.14)]">
        {clubsQuery.isLoading ? (
          ...
          <div className="px-8 py-16 text-center">
            <p className="font-semibold text-theme-heading">No clubs match your filters</p>
            <button
              type="button"
              onClick={() => setIsCreateOpen(true)}
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-theme-heading">{club.title}</p>
                            <span
                      ...
                      <td className="max-w-[220px] px-4 py-4">
                        <p className="line-clamp-2 text-xs leading-5 text-theme-muted">{desc}</p>
                      ...
                      <td className="px-4 py-4">
                        ...
                        <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[#f0f0ed]">
                          <div
                            className="h-full rounded-full bg-theme-button-primary"
                      ...
                      <td className="px-4 py-4">
                        <span className="font-semibold text-theme-heading">{club.eventCount ?? 0}</span>
        ...
        >
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#f0f0ed] pb-5">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-7 text-theme-muted">{selected.subtitle}</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <button
                type="button"
                onClick={() => setEditClub(selected)}
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div key={item.label} className="rounded-xl bg-theme-surface-muted px-4 py-3">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-[#9aa5a2]">
                  {item.label}
                </p>
                <p className="mt-1 font-semibold text-theme-heading">{item.value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 flex flex-wrap gap-4 text-sm text-theme-muted">
            <span>
              {selected.onboardingSteps.length} onboarding step
              {selected.onboardingSteps.length === 1 ? "" : "s"} (
              {selected.onboardingSteps.reduce((n, s) => n + s.questions.length, 0)} questions)
            </span>
            <span>·</span>
      ...
      <div className="grid gap-8 xl:grid-cols-2">
        <section>
          <div className="mb-4 flex items-center justify-between gap-4">
            <h2 className="text-lg font-semibold text-theme-heading">Club creation requests</h2>
            <span className="rounded-full bg-[#f0f0ed] px-3 py-1 text-xs font-semibold text-theme-muted">
          ...
          {creationQuery.isLoading ? (
            ...
            <p className="rounded-[20px] border border-dashed border-[#d5dbd8] bg-white px-6 py-10 text-center text-sm text-theme-muted">
            ...
            <div className="space-y-4">
              {creationRequests.map((req) => {
                ...
                return (
                  <article
                    key={req.id}
                    className="cursor-pointer rounded-[20px] border border-theme-muted bg-white p-5 shadow-[0_8px_28px_-20px_rgba(0,0,0,0.12)] transition hover:border-[#2f745f]/30"
                    onClick={() => setCreationReviewRequest(req)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter" || e.key === " ") {
                        e.preventDefault();
                        setCreationReviewRequest(req);
                      }
                    }}
                    role="button"
                    tabIndex={0}
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="flex min-w-0 flex-1 items-start gap-3">
                        {req.heroImageUrl ? (
                          <div
                            className="relative h-14 w-20 shrink-0 overflow-hidden rounded-xl border border-theme-muted bg-[#f0f0ed]"
                            aria-hidden
                          >
                        ...
                        <h3 className="font-semibold text-theme-heading">{req.title}</h3>
                      </div>
                      <button
                        type="button"
                        disabled={isBusy}
                        ...
                        className="rounded-full bg-theme-button-primary px-4 py-1.5 text-xs font-semibold text-white disabled:opacity-50"
                      >
                        Review
                      </button>
                    </div>
                    ...
                    <p className="mt-2 line-clamp-2 text-sm text-theme-muted">{requestDescription(req)}</p>
        ...
        <section className="rounded-[20px] border border-theme-muted bg-white p-5 shadow-[0_8px_28px_-20px_rgba(0,0,0,0.1)]">
          <div className="mb-4 flex items-center justify-between gap-4">
            <h2 className="text-lg font-semibold text-theme-heading">Member join requests</h2>
            <span className="rounded-full bg-[#f0f0ed] px-3 py-1 text-xs font-semibold text-theme-muted">
          ...
          {joinQuery.isLoading ? (
            ...
            <p className="py-8 text-center text-sm text-theme-muted">No pending membership requests.</p>
          ) : (
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm leading-6 text-[#3d4543]">
                          <span className="font-semibold text-theme-heading">{name}</span> wants to join{" "}
                          <button
                            type="button"
                            className="font-semibold text-theme-status-success hover:underline"
                            onClick={() => {
                        ...
                        <div className="mt-2 flex gap-2">
                          <button
                            ...
                            className="rounded-full bg-theme-button-primary px-3 py-1 text-xs font-semibold text-white disabled:opacity-50"
                          >
                            Approve
                          </button>
                          <button
          ...
          {joinRequests.length > 5 ? (
            <button
              ...
              className="mt-4 text-sm font-semibold text-theme-status-success hover:underline"
            >
      ...
      <AdminClubCreationReviewModal
        open={creationReviewRequest != null}
        request={creationReviewRequest}
        isPending={reviewCreation.isPending}
        onClose={() => setCreationReviewRequest(null)}
        onReview={(status) => {
          if (!creationReviewRequest) return;
          reviewCreation.mutate({ id: creationReviewRequest.id, status });
        }}
      />
    </div>
  );
